...
const App: React.FC = () => {
    ...
    const renderView = () => {
        ...
        switch (currentView) {
            ...
            case 'profile': return <ProfileView user={profileUser} isCurrentUser={profileUser?.id === currentUser?.id} currentUser={currentUser} onNavigate={(view, user) => handleNavigate(view, user)} onShowFollowers={u => handleOpenModal('followers', u)} onShowFollowing={u => handleOpenModal('following', u)} onViewPost={p => handleOpenModal('post', p)} onViewReel={r => handleOpenModal('reelViewer', r)} onFollow={handleFollowUser} onUnfollow={handleUnfollowUser} />;
