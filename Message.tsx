...
// Fix: Update ReplyPreview props to include currentUserId to fix incorrect type comparison.
const ReplyPreview: React.FC<{ message: MessageType, isCurrentUser: boolean, currentUserId: string }> = ({ message, isCurrentUser, currentUserId }) => {
    return (
        <div className={`text-xs opacity-80 mb-1 px-3 ${isCurrentUser ? 'text-right' : 'text-left'}`}>
            {/* Fix: Compare senderId with currentUserId instead of a boolean to determine if replying to self. */}
            <p className="text-gray-400">Replying to {message.senderId === currentUserId ? 'yourself' : 'them'}</p>
...
const Message: React.FC<MessageProps> = ({ message, currentUser, otherUser }) => {
  ...
  return (
    <div>
        {/* Fix: Pass currentUserId to ReplyPreview. */}
        {message.replyTo && <ReplyPreview message={message.replyTo} isCurrentUser={isCurrentUser} currentUserId={currentUser.id} />}
        <div 
            className={`flex items-end gap-2 group ${isCurrentUser ? 'justify-end' : 'justify-start'}`}
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
        >
        {!isCurrentUser && <img src={otherUser.avatar} alt={otherUser.username} className="w-8 h-8 rounded-full self-end" />}
        
        {isCurrentUser && (
            <div className={`flex items-center gap-1 transition-opacity duration-200 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
                <button className="p-1 rounded-full hover:bg-gray-700"><Icon className="w-5 h-5 text-gray-400"><path strokeLinecap="round" strokeLinejoin="round" d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" /></Icon></button>
                <button className="p-1 rounded-full hover:bg-gray-700"><Icon className="w-5 h-5 text-gray-400"><path d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></Icon></button>
            </div>
        )}
