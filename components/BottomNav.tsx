...
const BottomNav: React.FC<BottomNavProps> = ({ currentView, onNavigate, onCreate, currentUser }) => {
  const navItems = [
    ...
    { view: 'profile', icon: (active: boolean) => <img src={currentUser.avatar_url} alt="Profile" className={`w-7 h-7 rounded-full object-cover ring-2 ${active ? 'ring-white' : 'ring-transparent'}`} />, action: () => onNavigate('profile') },
  ];
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-sm border-t border-gray-800 flex justify-around p-1 md:hidden z-40">
      {navItems.map(item => (
