...
const ChoiceButton: React.FC<{ icon: React.ReactNode, label: string, onClick: () => void }> = ({ icon, label, onClick }) => (
    <button onClick={onClick} className="flex flex-col items-center justify-center p-6 bg-white border border-gray-200 hover:bg-gray-100 rounded-lg aspect-square transition-all transform hover:scale-105">
        <div className="w-12 h-12 text-blue-600">{icon}</div>
...
const CreateChoiceModal: React.FC<CreateChoiceModalProps> = ({ onClose, onChoice }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div 
        className="bg-gray-50 rounded-lg shadow-xl w-full max-w-lg border border-gray-200 flex flex-col animate-modal-intro"
