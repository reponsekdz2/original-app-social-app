          ...
          {reel.comments.map(comment => (
            <div key={comment.id} className="flex items-start gap-3">
              ...
              <button className="mt-2" onClick={() => onLikeComment(comment.id)}>
                <Icon className={`w-4 h-4 ${comment.likedBy.some(u => u.id === currentUser.id) ? 'text-red-500' : 'text-gray-400'}`} fill={comment.likedBy.some(u => u.id === currentUser.id) ? 'currentColor' : 'none'}>
