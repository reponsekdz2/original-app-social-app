...
const AdminView: React.FC<{ onExit: () => void }> = ({ onExit }) => {
    ...
    return (
        <div className="flex h-screen bg-gray-900 text-white">
            <aside className="w-20 lg:w-64 bg-black/50 p-4 flex flex-col border-r border-gray-700">
                <h1 className="text-2xl font-bold mb-8 text-red-500 hidden lg:block">Admin Panel</h1>
                <div className="w-8 h-8 text-red-500 mb-8 lg:hidden"><Icon fill="currentColor"><path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.321h5.367a.563.563 0 01.321.988l-4.338 3.14a.563.563 0 00-.184.55l1.637 5.111a.563.563 0 01-.812.622l-4.338-3.14a.563.563 0 00-.576 0l-4.338 3.14a.563.563 0 01-.812-.622l1.637-5.111a.563.563 0 00-.184-.55l-4.338-3.14a.563.563 0 01.321-.988h5.367a.563.563 0 00.475-.321L11.48 3.5z" /></Icon></div>
                <nav className="flex-1 space-y-2">
                    {navItems.map(item => (
                        <button key={item.id} onClick={() => setActiveView(item.id as AdminViewType)} className={`w-full flex items-center gap-4 p-3 rounded-lg text-left transition-colors ${activeView === item.id ? 'bg-gray-800' : 'hover:bg-gray-800'}`}>
                            <Icon className="w-6 h-6 flex-shrink-0">{item.icon}</Icon>
                            <span className="hidden lg:block">{item.label}</span>
                        </button>
                    ))}
                </nav>
                <button onClick={onExit} className="w-full flex items-center gap-4 p-3 rounded-lg text-left hover:bg-gray-800 mt-4">
                    <Icon className="w-6 h-6 flex-shrink-0"><path d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" /></Icon>
                    <span className="hidden lg:block">Exit Admin View</span>
                </button>
            </aside>
