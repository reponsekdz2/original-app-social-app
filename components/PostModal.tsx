...
const PostModal: React.FC<PostModalProps> = (props) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-0 md:p-4" onClick={onClose}>
      <div className="bg-gray-900 w-full h-full md:max-w-5xl md:h-full md:max-h-[90vh] flex flex-col md:flex-row md:rounded-lg shadow-2xl animate-modal-intro" onClick={e => e.stopPropagation()}>
        ...
        <div className="w-full md:w-2/5 flex flex-col">
          ...
          <footer className="p-4 border-t border-gray-700 space-y-3">
            <div className="flex items-center gap-4">
              <button onClick={() => onLike(post.id)}><Icon className={`w-7 h-7 ${isLiked ? 'text-red-500' : ''}`} fill={isLiked ? 'currentColor' : 'none'}><path d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></Icon></button>
            ...
            <form onSubmit={handleCommentSubmit} className="flex items-center gap-2 border-t border-gray-700 pt-3">
              ...
              <button type="submit" className="text-red-500 font-semibold text-sm disabled:text-gray-500" disabled={!commentText.trim()}>Post</button>
            </form>
          </footer>
        </div>
      </div>
