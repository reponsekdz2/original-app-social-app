...
const StepIndicator: React.FC<{ step: number, currentStep: number, label: string}> = ({ step, currentStep, label }) => {
    ...
    return (
        <div className="flex items-center gap-2">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${isCompleted ? 'bg-blue-600 text-white' : isActive ? 'bg-blue-700 text-white' : 'bg-gray-200 text-gray-500'}`}>
                {isCompleted ? <Icon className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></Icon> : step}
            </div>
            <span className={`font-semibold text-xs transition-colors ${isActive || isCompleted ? 'text-gray-800' : 'text-gray-400'}`}>{label}</span>
        </div>
    );
}
...
const RegisterForm: React.FC<RegisterFormProps> = ({ onRegisterSuccess, onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
   ...
   const SocialButton: React.FC<{ provider: 'Google' | 'Apple' }> = ({ provider }) => (
        <button type="button" className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-md font-semibold text-sm border border-gray-300 hover:bg-gray-100 transition-colors`}>
           {provider}
        </button>
    );
    ...
    <div className="flex flex-col h-full">
      ...
      <p className="text-center text-gray-500 mb-4 text-sm">Sign up to see photos and videos from your friends.</p>
       ...
       <div className="mb-4 px-2">
            <div className="flex items-center">
                ...
                <div className={`flex-1 h-0.5 mx-2 transition-colors ${step > 1 ? 'bg-blue-600' : 'bg-gray-200'}`} />
                <StepIndicator step={2} currentStep={step} label="Details" />
                <div className={`flex-1 h-0.5 mx-2 transition-colors ${step > 2 ? 'bg-blue-600' : 'bg-gray-200'}`} />
      ...
      <form onSubmit={handleSubmit} className="space-y-3 flex-grow flex flex-col">
            ...
            {step === 1 && (
                <div key="step1" className="space-y-3 animate-slide-fade-in">
                    ...
                    <div className="relative">
                        ...
                        <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" required autoFocus/>
                    </div>
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Username" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" required />
                    </div>
                </div>
            )}
            ...
            {step === 2 && (
                <div key="step2" className="space-y-3 animate-slide-fade-in">
                    ...
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Full Name" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" required autoFocus/>
                    </div>
                     <div className="relative">
                        ...
                        <input type="url" placeholder="Avatar URL (Optional)" value={avatarUrl} onChange={(e) => setAvatarUrl(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                    </div>
                    <div className="relative">
                        ...
                        <input type={showPassword ? 'text' : 'password'} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-10 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" required />
                    ...
                    <div className="relative">
                        <span className="absolute inset-y-0 left-0 flex items-center pl-3"><Icon className="w-5 h-5 text-gray-400"><path d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" /></Icon></span>
                        <input type={showConfirmPassword ? 'text' : 'password'} placeholder="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-10 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" required />
            ...
            {step === 3 && (
                <div key="step3" className="space-y-3 animate-slide-fade-in">
                    ...
                    <div className="relative">
                        ...
                        <input type="tel" placeholder="Phone Number (Optional)" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                    </div>
                    <div className="relative">
                        ...
                        <input type="text" onFocus={(e) => e.target.type='date'} onBlur={(e) => e.target.type='text'} placeholder="Date of Birth" value={dob} onChange={(e) => setDob(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" required />
                    </div>
                     <div className="relative">
                        <label className="text-xs text-gray-400 px-1">Gender</label>
                        <select value={gender} onChange={e => setGender(e.target.value as User['gender'])} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-3 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
                     ...
                     <div className="relative">
                        ...
                        <input type="text" placeholder="Country (Optional)" value={country} onChange={(e) => setCountry(e.target.value)} className="w-full bg-gray-100 border border-gray-300 rounded-md pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                    </div>
                </div>
            )}
        </div>
        ...
        <div className="mt-auto pt-4 flex gap-4">
             {step > 1 && (
                 <button type="button" onClick={handleBack} className="w-full bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-2.5 px-4 rounded-md">
                    Back
                </button>
             )}
            {step < 3 ? (
                ...
                <button type="submit" className="w-full btn-gradient text-white font-semibold py-2.5 px-4 rounded-md disabled:opacity-70 flex items-center justify-center" disabled={isLoading}>
