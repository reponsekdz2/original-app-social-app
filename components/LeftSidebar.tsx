            ...
            <nav className="flex-1 space-y-2">
                 {navItems.map(item => (
                    <button key={item.view} onClick={item.action ? item.action : () => onNavigate(item.view as View)} 
                        className={`w-full flex items-center gap-4 p-3 rounded-lg text-left transition-colors ${currentView === item.view ? 'bg-gray-100 font-bold text-blue-600' : 'hover:bg-gray-100 text-gray-800'}`}>
                        <div className={`w-7 h-7 ${currentView === item.view ? 'text-blue-600' : 'text-gray-800'}`}>{item.icon(currentView === item.view)}</div>
