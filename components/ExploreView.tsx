...
const ExploreView: React.FC<ExploreViewProps> = ({ posts }) => {
    ...
    <div className="pb-16 md:pb-4">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-1 md:gap-4">
        {shuffledPosts.map((post, index) => (
          <div key={`${post.id}-${index}`} className="relative aspect-square group cursor-pointer">
            {post.mediaType === 'video' ? (
                <video src={post.media} className="w-full h-full object-cover" />
            ) : (
                <img src={post.media} alt="Explore post" className="w-full h-full object-cover" />
            )}
