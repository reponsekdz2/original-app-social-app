  ...
  return (
    <>
      <div className="flex h-[calc(100vh-4rem)] md:h-screen bg-white">
        <div className={`w-full md:w-1/3 xl:w-1/4 border-r border-gray-200 flex-col ${selectedConvo ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className="overflow-y-auto flex-1">
            {conversations.map(convo => {
              ...
              return (
                <div key={convo.id} onClick={() => handleSelectConvo(convo)} className={`flex items-center gap-3 p-3 cursor-pointer ${selectedConvo?.id === convo.id ? 'bg-gray-100' : 'hover:bg-gray-50'}`}>
                  ...
                  <div>
                    ...
                    {lastMessage && <p className="text-sm text-gray-500 truncate w-48">{lastMessage.content}</p>}
                  </div>
                </div>
              );
            })}
        ...
        <div className={`w-full md:w-2/3 xl:w-3/4 ${selectedConvo ? 'block' : 'hidden md:block'}`}>
          {selectedConvo ? (
            ...
            <div className="flex items-center justify-center h-full text-gray-500 flex-col p-8 text-center">
              ...
              <h3 className="text-xl font-semibold mt-4 text-gray-800">Your Messages</h3>
              <p>Send private photos and messages to a friend or group.</p>
              <button onClick={() => setIsNewMessageModalOpen(true)} className="mt-4 bg-blue-600 text-white font-semibold py-2 px-4 rounded-md">
                Send Message
              </button>
            </div>
          )}
