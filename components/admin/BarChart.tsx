      ...
      {data.labels.map((label, index) => (
        <div key={label} className="flex-1 flex flex-col items-center justify-end h-full relative group">
            <div className="flex justify-center h-full w-full items-end gap-1">
                <div 
                    className="w-full bg-red-500 rounded-t-sm hover:bg-red-400 transition-colors relative"
                    style={{ height: `${(data.values[index] / maxValue) * 100}%` }}
                >
                     <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-xs font-bold opacity-0 group-hover:opacity-100">{data.values[index]}</span>
                </div>
                {data2 && (
                    <div 
                    ...
                    >
                        <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-xs font-bold opacity-0 group-hover:opacity-100">{data2.values[index]}</span>
                    </div>
                )}
            </div>
